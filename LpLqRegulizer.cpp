...
LpLqRegulizer::LpLqRegulizer(double pval, double qval) : p(pval), q(qval) {}

/**
 solve
 */
std::tuple<Vec, double, double> LpLqRegulizer::solve(Vec y, Mat A, Mat L,
                                                     double lambda) const {
  ...
  // initial estiMation
  Vec x_est = solve_l2l2_regularization(y, A, L, lambda2);
  ...
  while ((iteration <= MAX_ITER) && (diff >= tolerance)) {
    ...
    Vec x_est_new = solve_l2l2_regularization(bF, AF, LR, lambda2);
...
/**
 solve_l2l2_regularization
 */
Vec LpLqRegulizer::solve_l2l2_regularization(Vec y, Mat A, Mat L,
                                             double lambdasqr) const {
